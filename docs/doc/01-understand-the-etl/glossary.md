...
**ExternalFileItem**
: An item generated by `ExternalFileFinderConfig` for each remote file found. Carries the file's path and file system, and tracks whether the file has been moved to `processing/` or `processed/` by `ExternalFileProcessorConfig`. Not to be confused with `FileExtractedItem`, which is for locally-read files.

### F

**FileExtractedItem**
: An item that is generated after an operation has finished reading a local file (e.g. `CsvExtractConfig`, `JsonExtractConfig`). It signals that the file has been processed and can be archived or deleted.
...
**Flavor**
: A string identifying which registered factory to use when several share the same operation config class (e.g. one per remote storage). Defaults to `'default'`. See [Flavor](/doc/01-understand-the-etl/flavor.html).
...
**GenericChainFactory**
: A reflection-based factory that builds an operation from its config class, auto-wiring the operation's constructor. Registered once per operation (and per flavor, if needed) when setting up a `ChainBuilderV2`.
...
**OperationConfig**
: The typed object holding an operation's parameters (e.g. `CsvExtractConfig`, `RuleTransformConfig`). Every operation config implements `OperationConfigInterface`.
