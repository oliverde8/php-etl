...
sub-chains wrapped by `Split`, `Merge`, `Repeat`, `FailSafe`, and `If`. This means a parameter set with
...
`ChainSplitConfig`, `ChainMergeConfig`, `ChainRepeatConfig`, `FailSafeConfig`, and `IfConfig` all accept an
`isolateContext` option. When `true`, the sub-chain runs against its own clone of the context instead of the
shared one, so any parameters it sets are discarded once the operation finishes.
...
[If](/doc/20-operations/10-building/050-if.html#isolating-context).
