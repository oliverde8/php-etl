...
class ChainProcessorTest extends TestCase
{
    ...
    public function testReturnOfAsyncItems()
    {
        ...
        $mockEnd = new CallbackTransformerOperation(fn(ItemInterface $item) => $item);
    ...
    public function testAsyncItemsWithMixItem()
    {
        ...
        $mock = new CallbackTransformerOperation(fn(ItemInterface $item) => new MixItem([
    ...
    {
        $results = [];
        $mockBranch1 = new CallbackTransformerOperation(fn(ItemInterface $item) => new TestAsyncItem(new DataItem('I am slow'), 2));
        $mockBranch2 = new CallbackTransformerOperation(fn(ItemInterface $item) => new TestAsyncItem(new DataItem('I am fast'), 1));
        $mockEnd = new CallbackTransformerOperation(function (ItemInterface $item) use (&$results) {
            $results[] = $item->getData();
            return $item;
        });
    ...
    {
        $mock1 = new CallbackTransformerOperation(function (ItemInterface $item) use (&$count1): void {
